...
FROM_EMAIL = "EduAI <onboarding@resend.dev>"
    ...
    <div style="font-family: 'Segoe UI', Tahoma, sans-serif; max-width: 600px; margin: 0 auto;
      ...
      <div style="background: #ffffff; border-radius: 14px; overflow: hidden;">
        <div style="background: linear-gradient(135deg, #4f46e5, #7c3aed); padding: 24px 32px; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 24px;">EduAI Companion</h1>
        ...
        <div style="background: #f9fafb; padding: 16px 32px; text-align: center; border-top: 1px solid #e5e7eb;">
          <p style="color: #9ca3af; font-size: 12px; margin: 0;">
            EduAI Companion | DSGVO-konform | <a href="#" style="color: #6366f1;">Abmelden</a>
          </p>
        </div>
      </div>
    </div>
...
async def send_welcome_email(to: str, username: str) -> bool:
    ...
    content = f"""
    <h2 style="color: #1f2937;">Willkommen bei EduAI, {username}!</h2>
    <p style="color: #4b5563; line-height: 1.6;">
      Schoen, dass du dabei bist! Mit EduAI hast du jetzt Zugang zu:
    </p>
    ...
    <div style="text-align: center; margin: 24px 0;">
      <a href="https://eduai-companion.vercel.app" style="display: inline-block; background: linear-gradient(135deg, #4f46e5, #7c3aed);
    ...
    """
    return await _send_email(to, f"Willkommen bei EduAI, {username}!", _base_template(content))
...
async def send_password_reset_email(to: str, username: str, reset_token: str) -> bool:
    ...
    """
    return await _send_email(to, "Passwort zuruecksetzen - EduAI", _base_template(content))
...
async def send_parent_link_email(
    ...
    <div style="text-align: center; margin: 24px 0;">
      <a href="https://eduai-companion.vercel.app/verify-parent?token={verify_token}" style="display: inline-block;
         background: #22c55e; color: white; padding: 12px 32px; border-radius: 8px; text-decoration: none;
         font-weight: 600; margin-right: 12px;">
        Bestaetigen
      </a>
      <a href="https://eduai-companion.vercel.app/reject-parent?token={verify_token}" style="display: inline-block;
...
async def send_weekly_report_email(
    ...
    return await _send_email(
        to,
        f"Wochen-Report: {xp} XP, {quizzes} Quizze - EduAI",
        _base_template(content),
    )
...
async def send_streak_loss_email(to: str, username: str, lost_streak: int) -> bool:
    ...
    return await _send_email(
        to,
        f"Dein {lost_streak}-Tage Streak ist vorbei - EduAI",
        _base_template(content),
    )
...
async def send_tournament_winner_email(
    ...
    return await _send_email(
        to,
        f"Platz {rank} im {subject}-Turnier! - EduAI",
        _base_template(content),
    )
