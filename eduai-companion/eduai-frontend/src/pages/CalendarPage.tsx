    ...
    <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Plane deine Klausuren und lass dir einen KI-Lernplan erstellen</p>
        </div>
      ...
      {exams.length === 0 ? (
        ...
        <div className="space-y-4">
          {exams.map((exam) => {
            ...
            const urgent = days <= 3;
            return (
              <Card key={exam.id} className={urgent ? "border-red-200 dark:border-red-800" : ""}>
                      ...
                      <div className="space-y-2">
                        {activePlan.days.map((day, i) => (
                          <div key={i} className="flex items-center gap-3 text-sm">
                            <span className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-700 dark:text-indigo-300 font-medium text-xs shrink-0">
                              {day.tag}
                            </span>
                            <span className="text-gray-600 dark:text-gray-400 w-20 shrink-0">
                              {day.datum ? new Date(day.datum).toLocaleDateString("de-DE", { weekday: "short", day: "numeric", month: "short" }) : `Tag ${day.tag}`}
                            </span>
                            <span className="text-gray-900 dark:text-white flex-1">{day.aufgabe}</span>
