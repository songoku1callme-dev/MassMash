...
export default function ShopPage() {
 ...
 <p className="theme-text-secondary">Gib deine XP für Themes, KI-Stile und Boosts aus</p>
 </div>
 <div className="text-right">
 <p className="text-3xl font-bold text-yellow-600">{userXp}</p>
 <p className="text-xs theme-text-secondary">Verfügbare XP</p>
 </div>
 </div>
 ...
 <div
 key={item.id}
 className={`relative theme-card rounded-xl overflow-hidden border transition-all ${
 item.unlocked
 ? "border-green-300"
 ...
 <h3 className="font-semibold theme-text mb-1">{item.name}</h3>
 <p className="text-xs theme-text-secondary mb-3">
 {CATEGORY_LABELS[item.category] || item.category}
 </p>

 <div className="flex items-center justify-between">
 <span className="text-lg font-bold text-yellow-600">{item.price} XP</span>
 {!item.unlocked && (
 <Button
 size="sm"
