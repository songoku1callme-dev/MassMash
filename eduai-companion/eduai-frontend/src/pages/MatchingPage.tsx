...
export default function MatchingPage() {
 ...
 <p className="text-sm font-medium text-cyan-700 mb-2">Deine schwachen Fächer:</p>
 <div className="flex flex-wrap gap-2">
 {myWeakSubjects.map((s) => (
 <span key={s} className="px-3 py-1 bg-cyan-100 text-cyan-700 text-sm rounded-full">
 {s}
 </span>
 ))}
 </div>
 ...
 disabled={loading}
 className="flex items-center gap-2 px-4 py-2 text-sm text-cyan-700 bg-cyan-50 rounded-lg hover:bg-cyan-100 transition-colors"
 >
 ...
 <div key={partner.user_id} className="p-5 theme-card rounded-xl border border-[var(--border-color)] hover:shadow-md transition-all">
 ...
 <h3 className="font-semibold theme-text">{partner.username}</h3>
