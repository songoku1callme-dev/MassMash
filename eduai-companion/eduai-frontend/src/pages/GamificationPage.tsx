  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">Dein Fortschritt, Achievements und Rangliste</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">Level {profile.level}</p>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">{profile.streak_days}</p>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">{profile.quizzes_completed}</p>
        ...
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
              tab === t.id ? "border-blue-500 text-blue-600 dark:text-blue-400" : "border-transparent text-gray-500 hover:text-gray-700"
            }`}>
            {t.icon} {t.label}
          </button>
        ))}
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                  <span className="text-sm text-gray-700 dark:text-gray-300">Quizzes abgeschlossen</span>
                  <Badge variant="secondary">{profile.quizzes_completed}</Badge>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                  <span className="text-sm text-gray-700 dark:text-gray-300">Chat-Nachrichten</span>
                  <Badge variant="secondary">{profile.chats_sent}</Badge>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                  <span className="text-sm text-gray-700 dark:text-gray-300">Abitur-Simulationen</span>
              ...
              <div className="space-y-2">
                {profile.all_levels.map((lvl) => (
                  <div key={lvl.level} className={`flex items-center justify-between p-2 rounded-lg text-sm ${
                    profile.level >= lvl.level ? "bg-yellow-50 dark:bg-yellow-900/20" : "bg-gray-50 dark:bg-gray-800/50 opacity-60"
                  }`}>
      ...
      {tab === "achievements" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {profile.all_achievements.map((ach) => {
            ...
            return (
              <Card key={ach.id} className={earned ? "border-yellow-300 dark:border-yellow-700" : "opacity-60"}>
                <CardContent className="p-4 flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center text-2xl ${
                    earned ? "bg-yellow-100 dark:bg-yellow-900/30" : "bg-gray-100 dark:bg-gray-800"
                  }`}>
                    {ach.emoji}
                  </div>
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-yellow-600 dark:text-yellow-400 mt-1">+{ach.xp_reward} XP</p>
                  </div>
                </CardContent>
              </Card>
            );
              ...
              <div className="space-y-2">
                {leaderboard.map((entry) => (
                  <div key={entry.rank} className={`flex items-center gap-3 p-3 rounded-lg ${
                    entry.is_you ? "bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800" : "bg-gray-50 dark:bg-gray-800/50"
                  }`}>
                    ...
                    <div className="text-right">
                      <p className="font-bold text-gray-900 dark:text-white">{entry.xp} XP</p>
