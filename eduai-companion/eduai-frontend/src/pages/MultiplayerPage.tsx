...
export default function MultiplayerPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto p-6 space-y-8">
        <div className="text-center">
          ...
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Multiplayer-Quiz</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-2">Fordere deine Freunde heraus!</p>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Raum erstellen</h2>
            <select value={subject} onChange={(e) => setSubject(e.target.value)} className="w-full p-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm mb-4">
  ...
  if (gameStatus === "waiting") {
    return (
      <div className="max-w-lg mx-auto p-6 space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Warteraum</h2>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-3">Spieler ({players.length}/8)</h3>
  ...
  if (gameStatus === "playing" && questions.length > 0) {
    ...
    return (
      <div className="max-w-2xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-6">{q.frage}</h2>
          <div className="grid grid-cols-1 gap-3">
            {q.optionen.map((opt, idx) => (
              <button key={idx} onClick={() => submitAnswer(idx)} disabled={!!result}
                className={`p-4 rounded-xl text-left font-medium transition-all border-2 ${result ? "border-gray-200 dark:border-gray-600" : "border-gray-200 dark:border-gray-600 hover:border-purple-500 hover:bg-purple-50 dark:hover:bg-purple-900/20"}`}>
        ...
        {result && (
          <div className={`p-4 rounded-xl ${result.correct ? "bg-green-50 dark:bg-green-900/30 border border-green-200" : "bg-red-50 dark:bg-red-900/30 border border-red-200"}`}>
            ...
            {result.erklaerung && <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">{result.erklaerung}</p>}
          </div>
        )}
        {scores.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700">
  ...
  return (
    <div className="max-w-lg mx-auto p-6 space-y-6 text-center">
      ...
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Quiz beendet!</h1>
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
        <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Endstand</h3>
